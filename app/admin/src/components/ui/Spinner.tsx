...
export default function Spinner({ inline, overlay, text, size = 14 }: SpinnerProps) {
  ...
  if (overlay) {
    return (
      <div role="status" aria-live="polite" className="fixed inset-0 z-[9999] flex items-center justify-center bg-background/90">
        {content}
      </div>
    );
  }
  ...
  return (
    <div role="status" aria-live="polite" className="flex h-full min-h-50 items-center justify-center px-4 py-6">
      {content}
    </div>
  );
}
