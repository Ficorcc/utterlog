...
- **通知邮件的视觉统一为直角、去掉符号图标**：状态标记原先是方块里放对号、省略号、垃圾桶等符号，各家邮件客户端的字形和基线差异很大，垃圾桶在部分 Windows 客户端还会渲染成彩色图案；现在一律改为「已通过」「待审核」这样的中文字样。头像此前在回复通知里是圆形、新评论通知里是方形，连同卡片、按钮、徽章一起统一为直角。
